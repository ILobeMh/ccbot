...
| `CCBOT_NOTIFY_INPUT` / `_DONE` / `_LIFECYCLE` / `_ERRORS` / `_CCC` | `true` | Notification kinds posted to the `notifications` topic |
| `CCBOT_NOTIFY_DONE_MIN_SECONDS` | `0` | Only announce finished turns that took at least this long |
...
| `CCBOT_SPECIAL_TOPICS` | `shell,ccc,settings,notifications` | Bot-owned topics to create in the group (see below); empty disables |
...
| `notifications` | One feed for events from every session, each with a button that jumps to its topic: ❓ needs input (question, permission / plan prompt, dialog — once per dialog), ✅ turn finished (with duration and the reply's first line; optional minimum duration), 🔄 lifecycle (created / resumed / restarted / exited / update installed), 🚨 API errors from the transcript (session limit, rate limit, 5xx), 🔀 ccc quota. Each kind toggles in `settings`; quiet hours apply. |
